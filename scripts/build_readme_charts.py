...
CASH = load_cash_rate()
THRESHOLDS = f"{DEFAULT_ENTRY_THRESHOLD:g}/{DEFAULT_EXIT_THRESHOLD:g}"

data = load_data("TQQQ")
result = run_backtest(data, cash_rate=CASH)
...
plot_equity(
    result, ax=axes[0],
    title=f"TQQQ 2010-2026 - IBS {THRESHOLDS} vs buy & hold (start = 100)",
)
...
wf = walk_forward(data, objective="cagr", cash_rate=CASH)
ax = plot_walk_forward(wf)
save(ax.figure, "walkforward.png")

# 3. Extended 1999+ history at the defaults, log scale -- the window the
# defaults were chosen on, and the one where buy & hold is nearly wiped out
extended = load_extended_data("TQQQ", "QQQ")
crash_result = run_backtest(extended, cash_rate=CASH)
ax = plot_equity(
    crash_result,
    log=True,
    title=f"1999-2026 - IBS {THRESHOLDS} vs buy & hold (log scale)",
)
